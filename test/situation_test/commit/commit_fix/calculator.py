...
def divide(a: float, b: float) -> float:
    ...
    if b == 0:
        if a == 0:
            raise ZeroDivisionError("Forme indéterminée 0/0 non calculable")
        raise ZeroDivisionError("Division par zéro impossible")
    return a / b


def format_result(val: float) -> str:
    """Nettoie l'affichage du résultat (supprime -0.0 et arrondit les imprécisions flottantes)."""
    val = round(val, 10)
    if val == 0:
        return "0.0"
    return str(val)
...
def main():
    ...
    try:
        ...
        print(f"Résultat: {format_result(result)}")
